...
function validateAndProcessLabels(data, format) {
  if (!Array.isArray(data)) {
    throw new Error(`${format.toUpperCase()} file must contain an array of label objects`);
  }
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!item.name || typeof item.name !== 'string' || item.name.trim() === '') {
      results.errors.push(`Item at index ${i} has invalid or missing 'name' field`);
      continue;
    }

    // Validate optional color field
    if (item.color !== undefined && (typeof item.color !== 'string' || item.color.trim() === '')) {
    ...
    const unknownFields = Object.keys(item).filter(key => !knownFields.includes(key));
    if (unknownFields.length > 0) {
      results.warnings.push(`Item at index ${i} contains unknown fields: ${unknownFields.join(', ')}`);
    }
    ...
    if (item.description !== undefined) cleanLabel.description = item.description;
...
async function testFormatCompatibility(testCase) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      log(colorize('blue', `   Valid labels: ${jsonResults.validLabels.length}`));
      ...
      log(colorize('blue', `   Valid labels: ${yamlResults.validLabels.length}`));
      ...
      if (resultsMatch) {
        ...
        if (labelsMatch) {
          ...
          resolve({ success: true, validLabels: jsonResults.validLabels.length });
        } else {
        ...
        log(colorize('red', `   JSON: ${jsonResults.validLabels.length} valid, ${jsonResults.errors.length} errors, ${jsonResults.warnings.length} warnings`));
        log(colorize('red', `   YAML: ${yamlResults.validLabels.length} valid, ${yamlResults.errors.length} errors, ${yamlResults.warnings.length} warnings`));
...
async function testJsonBackwardCompatibility() {
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(parsedData)) {
          ...
          log(colorize('blue', `   Valid labels: ${results.validLabels.length}`));
          log(colorize('red', `   Errors: ${results.errors.length}`));
          log(colorize('yellow', `   Warnings: ${results.warnings.length}`));
        } else {
          log(colorize('yellow', '⚠️  Non-array structure (expected for some test files)'));
        }
        
        passedTests++;
      } catch (parseError) {
        log(colorize('yellow', `⚠️  Parse error (expected for some test files): ${parseError.message}`));
  ...
  log(colorize('green', `✅ JSON backward compatibility: ${passedTests}/${totalTests} tests passed`));
...
async function runCompatibilityTests() {
  log(
    colorize(
      'blue',
      '🧪 Testing JSON-YAML Compatibility',
    ),
  );
  ...
  log(colorize('green', `✅ Format compatibility: ${passedTests}/${totalTests} tests passed`));
  log(colorize('green', `✅ JSON backward compatibility: ${jsonCompatible ? 'PASSED' : 'FAILED'}`));
