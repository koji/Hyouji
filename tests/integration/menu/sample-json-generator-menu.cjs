...
function testMenuOptionDisplay() {
  ...
  try {
    ...
    if (exitMatch && parseInt(exitMatch[1]) !== 7) {
      throw new Error(`Expected exit option value 7, but found ${exitMatch[1]}`)
    }
...
function testMenuSelectionFunctionality() {
  ...
  try {
    ...
    console.error('❌ Menu selection functionality test failed:', error.message)
    throw error
  }
}
...
function testApplicationFlow() {
  ...
  try {
    ...
    if (
      ...
      throw new Error('Recursive main() call not found at end of main function')
    }
...
function testEndToEndFlow() {
  ...
  try {
    ...
    console.log('🔍 Verifying built application includes generateSampleJson...')
