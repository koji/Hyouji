...
const configurationTests = [
  {
    ...
    type: 'error-handling',
    requirements: ['Configuration error recovery', 'User-friendly error messages'],
  },
  {
    ...
    type: 'validation',
    requirements: ['Config validation', 'Error message quality', 'Recoverable error detection'],
  },
];
...
async function executeIndividualTest(testName) {
  ...
  if (!test) {
    ...
    configurationTests.forEach((t, index) => {
      log(colorize('gray', `  ${index + 1}. ${t.name} (${t.type}) - ${t.file}`));
    });
    return false;
  }
...
async function executeTestsByType(type) {
  const testsOfType = configurationTests.filter(t =>
...
async function executeAllTests() {
  ...
  log(colorize('blue', '='.repeat(50)));
  log(colorize('gray', 'Executing all configuration tests with consolidated reporting'));
  log('');
  ...
  if (passedTests < totalTests) {
    log(colorize('red', `❌ Failed: ${totalTests - passedTests}/${totalTests} tests`));
  }
