...
const verificationScripts = [
  {
    ...
    description:
      'Manual verification script for configuration error handling scenarios',
    type: 'config',
    requirements: [
...
function discoverVerificationScripts() {
  ...
  for (const item of items) {
    ...
    if (
      stat.isFile() &&
      (item.endsWith('.js') || item.endsWith('.cjs') || item.endsWith('.mjs'))
    ) {
      // Skip .gitkeep files and already included scripts
      if (item === '.gitkeep' || item === 'run-all.cjs') continue

      const alreadyIncluded = verificationScripts.some(
        (script) => script.file === fullPath,
      )
  ...
  return new Promise((resolve) => {
    ...
    if (script.interactive) {
      log(
        colorize(
          'yellow',
          `   ⚠️  Interactive script - may require manual steps`,
        ),
      )
    }
...
async function executeScriptsByType(type) {
  const allScripts = [...verificationScripts, ...discoverVerificationScripts()]
  const scriptsOfType = allScripts.filter(
...
async function executeAllScripts() {
  ...
  log(
    colorize(
      'gray',
      'Executing all verification scripts with consolidated reporting',
    ),
  )
  log('')
  ...
  Object.keys(resultsByType).forEach((type) => {
    ...
    log(
      colorize(
        'cyan',
        `📋 ${type.toUpperCase()} Verification (${typeCompleted}/${typeResults.length} completed):`,
      ),
    )
    typeResults.forEach((result, index) => {
      ...
      if (result.script.interactive) {
        log(
          colorize(
            'gray',
            `     Note: Interactive script - may require manual verification`,
          ),
        )
      }
  ...
  log(
    colorize(
      'green',
      `✅ Completed: ${completedScripts}/${totalScripts} scripts`,
    ),
  )
  ...
  if (completedScripts < totalScripts) {
    log(
      colorize(
        'red',
        `❌ Failed: ${totalScripts - completedScripts}/${totalScripts} scripts`,
      ),
    )
  }
  ...
  if (completedScripts === totalScripts) {
    ...
    types.forEach((type) => {
      ...
      log(
        colorize(
          'gray',
          `  • ${type}: ${typeCompleted}/${typeScripts.length} scripts`,
        ),
      )
    })
    ...
    log(
      colorize(
        'yellow',
        '📝 Note: Some scripts may be interactive and require manual verification.',
      ),
    )
    log(
      colorize(
        'yellow',
        '    Please review the output above to ensure all requirements are met.',
      ),
    )
  } else {
    log('')
    log(
      colorize(
        'yellow',
        '⚠️  Some verification scripts failed. Please review the output above.',
      ),
    )
  }
...
function displayHelp() {
  ...
  log(
    '  Some verification scripts are interactive and may require manual steps.',
  )
  ...
  Object.keys(typeGroups).forEach((type) => {
    ...
    typeGroups[type].forEach((script, index) => {
      ...
      log(
        colorize(
          'gray',
          `     Requirements: ${script.requirements.join(', ')}`,
        ),
      )
      if (script.interactive) {
