...
content = content.replace(
  /mockExec\.mockImplementation\(\s*\([^)]*\)\s*=>\s*\{[^}]*if\s*\([^)]*callback[^)]*\)\s*\{[^}]*process\.nextTick\(\(\)\s*=>\s*callback\(null,\s*([^,]+),\s*([^)]+)\)\);?\s*\}[^}]*return[^}]*\}\s*\)/gs,
  (match, stdout, stderr) => {
...
content = content.replace(
  /mockExec\.mockImplementation\(\s*\([^)]*\)\s*=>\s*\{[^}]*if\s*\([^)]*callback[^)]*\)\s*\{[^}]*process\.nextTick\(\(\)\s*=>\s*callback\(([^,)]+)[^)]*\)\);?\s*\}[^}]*return[^}]*\}\s*\)/gs,
  (match, error) => {
